...
describe('Utils', () => {
  ...
  describe('LazyLoading', () => {
    it('creates intersection observer for lazy loading', () => {
      ...
      lazyLoading.observeElements(elements);
    ...
    it('loads images when they enter viewport', () => {
      ...
      img.dataset.srcset = '/images/test-480w.jpg 480w, /images/test-768w.jpg 768w';
      
      lazyLoading.loadImage(img);
      
      expect(img.src).toBe('/images/test.jpg');
      expect(img.srcset).toBe('/images/test-480w.jpg 480w, /images/test-768w.jpg 768w');
    ...
    it('loads background images correctly', () => {
      ...
      lazyLoading.loadBackgroundImage(div);
      
      expect(div.style.backgroundImage).toBe('url(/images/background.jpg)');
    ...
    it('preloads images correctly', async () => {
      ...
      const loadPromise = lazyLoading.preloadImage(imageSrc);
    ...
    it('provides fallback for images that fail to load', () => {
      ...
      lazyLoading.handleImageError(img, '/images/fallback.jpg');
      
      expect(img.src).toBe('/images/fallback.jpg');
  ...
  describe('NetworkErrorHandler', () => {
    ...
    it('handles fetch errors correctly', async () => {
      ...
      const result = await networkErrorHandler.handleFetchError('/api/test');
    ...
    it('implements retry logic correctly', async () => {
      ...
      const result = await networkErrorHandler.retryRequest('/api/test', { maxRetries: 3 });
    ...
    it('handles timeout errors correctly', async () => {
      ...
      const result = await networkErrorHandler.handleTimeout('/api/test', 100);
    ...
    it('provides offline fallback correctly', () => {
      ...
      const result = networkErrorHandler.getOfflineFallback('test-key', fallbackData);
  ...
  describe('Performance', () => {
    ...
    it('provides performance budget monitoring', () => {
      ...
      const budgetStatus = performance.checkPerformanceBudget(budget, currentUsage);
