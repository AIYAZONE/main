...
export interface SkillService {
  ...
  getSkillsByCategory(categoryType: 'frontend' | 'management' | 'leadership'): Promise<SkillCategory>;
...
class SkillServiceImpl implements SkillService {
  ...
  async getSkillCategories(): Promise<SkillCategory[]> {
    try {
      ...
      return skillsData.skillCategories as SkillCategory[];
    } catch (error) {
      throw new Error(`Failed to fetch skill categories: ${error}`);
    }
  }

  async getSkillsByCategory(categoryType: 'frontend' | 'management' | 'leadership'): Promise<SkillCategory> {
    try {
  ...
  async searchSkills(query: string): Promise<Skill[]> {
    try {
      ...
      return allSkills.filter(skill => 
        ...
        skill.projects.some(project => project.toLowerCase().includes(lowercaseQuery))
      );
    } catch (error) {
  ...
  getSkillsSummary(skills: SkillCategory[]): {
    ...
    const averageLevel = allSkills.reduce((sum, skill) => sum + skill.level, 0) / totalSkills;
    const topSkills = allSkills
      .filter(skill => skill.level >= 8)
      .sort((a, b) => b.level - a.level)
      .slice(0, 5);
    const experienceYears = Math.max(...allSkills.map(skill => skill.yearsOfExperience));
