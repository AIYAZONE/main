...
class CacheService {
   ...
   * 设置缓存
   */
  set<T>(key: string, data: T, optionsOrTtl: CacheOptions | number = {}): void {
   ...
   * 删除缓存
   */
  remove(key: string, storage: 'memory' | 'localStorage' | 'sessionStorage' = 'memory'): void {
    switch (storage) {
      case 'memory':
...
export const cache = {
  ...
  get: <T>(key: string, storage?: 'memory' | 'localStorage' | 'sessionStorage') => 
    cacheService.get<T>(key, storage),
  
  delete: (key: string, storage?: 'memory' | 'localStorage' | 'sessionStorage') => 
    cacheService.remove(key, storage),
  
  clear: (storage?: 'memory' | 'localStorage' | 'sessionStorage') => 
    cacheService.clear(storage),
  
  has: (key: string, storage?: 'memory' | 'localStorage' | 'sessionStorage') => 
...
export const CACHE_CONFIGS = {
  ...
  PERSISTENT: { ttl: 7 * 24 * 60 * 60 * 1000, storage: 'localStorage' as const } // 7天，本地
} as const;