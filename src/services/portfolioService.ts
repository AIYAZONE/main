import type { ProjectGroup, Project, ProjectDetail, FilterCriteria } from '../types/portfolio';
...
class PortfolioServiceImpl implements PortfolioService {
    ...
    return cache.getOrSet(
      ...
      async () => {
        try {
          ...
          return portfolioData.projectGroups as ProjectGroup[];
        } catch (error) {
    ...
    return cache.getOrSet(
      ...
      async () => {
        try {
          ...
          const project = projects.flatMap(group => group.projects).find(p => p.id === id);
          ...
          return {
            ...project,
            fullDescription: `${project.description} - 这是一个详细的项目描述，包含了项目的背景、目标、实现方案和技术细节。`,
            screenshots: [
  ...
  async syncExternalProjects(): Promise<void> {
    try {
      ...
      const projects = await this.getProjects();
      const projectsWithGitHub = projects.flatMap(group => group.projects)
        .filter(project => project.githubUrl);
      
      console.log(`Found ${projectsWithGitHub.length} projects with GitHub URLs to sync`);
      ...
      for (const project of projectsWithGitHub) {
        if (project.metrics) {
          ...
          const mockGitHubData = {
            stars: project.metrics.stars || Math.floor(Math.random() * 100) + 10,
            forks: project.metrics.forks || Math.floor(Math.random() * 20) + 1,
            updated_at: new Date().toISOString()
          };
          
          // Mock analytics data
          const mockAnalyticsData = {
            visitors: project.metrics.visitors || Math.floor(Math.random() * 5000) + 100
          };
          
          console.log(`Updated ${project.name}: ${mockGitHubData.stars} stars, ${mockGitHubData.forks} forks, ${mockAnalyticsData.visitors} visitors`);
        }
      }
  ...
  categorizeProjectsForDisplay(projectGroups: ProjectGroup[]): {
    ...
    const digitalGarden = projectGroups.find(group => group.id === 'digital-garden');
    const featuredWork = projectGroups.find(group => group.id === 'featured-work');
    const community = projectGroups.find(group => group.id === 'community');
  ...
  getProjectsByCategory(projectGroups: ProjectGroup[], category: 'digital-garden' | 'featured-work' | 'community'): Project[] {
  ...
  getProjectStatistics(projectGroups: ProjectGroup[]): {
    ...
    const onlineProjects = allProjects.filter(p => p.status === 'online').length;
    const developmentProjects = allProjects.filter(p => p.status === 'development').length;
    
    const totalStars = allProjects.reduce((sum, project) => 
      sum + (project.metrics?.stars || 0), 0);
    const totalVisitors = allProjects.reduce((sum, project) => 
  ...
  getFeaturedProjects(projectGroups: ProjectGroup[]): Project[] {
    ...
    return allProjects
      ...
      .sort((a, b) => {
        const aScore = (a.metrics?.stars || 0) + (a.metrics?.visitors || 0) / 100;
        const bScore = (b.metrics?.stars || 0) + (b.metrics?.visitors || 0) / 100;
  ...
  getProjectById(projectGroups: ProjectGroup[], projectId: string): Project | null {
  ...
  getRelatedProjects(projectGroups: ProjectGroup[], currentProject: Project, limit: number = 3): Project[] {
    const allProjects = projectGroups.flatMap(group => group.projects)
    ...
    return allProjects
      .map(project => ({
        project,
        similarity: this.calculateTechStackSimilarity(currentProject.techStack, project.techStack)
      }))
  ...
  private calculateTechStackSimilarity(stack1: string[], stack2: string[]): number {
    const set1 = new Set(stack1.map(tech => tech.toLowerCase()));
    const set2 = new Set(stack2.map(tech => tech.toLowerCase()));
    
    const intersection = new Set([...set1].filter(tech => set2.has(tech)));
