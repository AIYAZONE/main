...
export interface ErrorInfo {
  ...
  category: 'network' | 'runtime' | 'resource' | 'user' | 'system' | 'validation' | 'generic';
}
...
class ErrorHandlingService {
   ...
   * 处理错误
   */
  handleError(error: Error | (Partial<ErrorInfo> & { message: string }), options?: { log?: boolean; report?: boolean }): any {
    ...
    // 用户通知
    if (this.options.enableUserNotification && errorInfo.severity === 'critical') {
  ...
  async withFallback<T>(
    ...
    try {
      ...
      this.handleError({
        message: `Primary operation failed, using fallback: ${(error as Error).message}`,
  ...
  getErrorStats(): {
    ...
    const recent = this.errors
      .filter(error => Date.now() - error.timestamp < 24 * 60 * 60 * 1000) // 24小时内
  ...
  private logError(error: ErrorInfo): void {
    ...
    console[logMethod](`[${error.category.toUpperCase()}] ${error.message}`, {
      id: error.id,
...
export const errorHandler = {
  ...
  component: (error: Error, componentName: string, context?: Record<string, any>) => 
  ...
  withFallback: <T>(primary: () => Promise<T>, fallback: () => Promise<T> | T, options?: any) => 
